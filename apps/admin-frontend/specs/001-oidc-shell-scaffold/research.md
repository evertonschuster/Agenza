...
- **Decision**: A small `shared/logger.ts` wrapping `console` (structured — event name + minimal metadata, no PII beyond tenant id), called from `logAuthEvent` in `features/auth/model/sessionStore.ts` on login outcome, renewal failure, and logout (spec FR-015). No external telemetry/observability backend.
...
- **Decision**: The Playwright e2e suite performs an actual interactive browser login using the seeded demo user (`owner@demo.local` / `Passw0rd!`), which `identity-service` provisions automatically when `DemoTenant` config is present (already true in `appsettings.Development.json`). The tenant id itself is generated fresh (`Guid.CreateVersion7()`) at seed time rather than read from config, so it differs per environment — the e2e suite reads it from the issued access token instead of asserting a fixed value.
