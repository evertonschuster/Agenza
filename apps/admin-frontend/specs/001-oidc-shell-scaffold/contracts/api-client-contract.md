...
- `src/shared/api/settle.ts` — `settle(call)` wraps a repository promise so the UI always gets a **renderable** `ApiResult`, instead of every page restating `try { … } catch { setProblem(OFFLINE) }`. A transport rejection (offline, DNS, connection refused) becomes `NETWORK_PROBLEM`; a non-2xx whose body isn't valid `problem+json` (a gateway 5xx, an empty error body) becomes `SERVER_PROBLEM`. Both are `ApiProblem`s with `status: 0` and a namespaced `code`, so the interface layer branches on them exactly like a real backend Problem. The facade is left untouched — it still returns only the backend's two shapes; `settle` is the opt-in boundary above it.
...
`Result` / `ok` / `fail` live in `src/shared/result.ts`. The interface layer awaits the repository through `settle`, then branches on `result.ok`, then on `result.error.code` (or `.status`), rendering `result.error.title` and reading `result.error.errors` directly.
