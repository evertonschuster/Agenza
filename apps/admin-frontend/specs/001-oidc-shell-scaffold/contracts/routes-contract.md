...
|---|---|---|---|
| `/login` | Public | `LoginPage` component (`ui/pages/LoginPage/`): triggers the OIDC redirect to identity-service's `/connect/authorize` via its `useLoginRedirect` hook | Also the post-logout redirect target (`VITE_OIDC_POST_LOGOUT_REDIRECT_URI`). Does not itself render a form — identity-service owns the actual login UI (constitution: auth goes through identity-service). |
| `/callback` | Public (OIDC round-trip only) | `AuthCallbackPage` component (`ui/pages/AuthCallbackPage/`): its `useAuthCallback` hook completes `oidc-client-ts`'s `signinCallback()`, then navigates to `/` | Fixed by `VITE_OIDC_REDIRECT_URI`; MUST exactly match the registered redirect URI in identity-service's `admin-panel` client registration. |
