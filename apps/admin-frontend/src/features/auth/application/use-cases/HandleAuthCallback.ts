 ...
 * a new exchange.
 *
 * Deliberately does not catch or wrap errors from the repository: the
 * exact error shapes oidc-client-ts produces against a real IdentityServer
 * (expired code, denied consent, network error) haven't been observed yet,
 * so swallowing or re-wrapping them now would discard information
 * presentation might need later (docs/DECISIONS.md). Revisit once those
 * real error shapes can be confirmed against the backend.
 */
