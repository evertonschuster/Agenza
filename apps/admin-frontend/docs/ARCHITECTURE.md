...
| `shared/pubsub/` added (`createTopic`/`useTopic`); the row above's `reload` handed through `<Outlet context>` is replaced, not kept alongside, by `TagDeletePage` publishing to a `tagDeleted` topic owned by `features/tags/model/tagEvents.ts` | Explicit ask, not a default reached for on its own — the outlet-context callback directly coupled the delete route to the list page's internals, and the request was specifically for something reusable by other modules with zero knowledge of tags. `createTopic<T>()` is a factory, not one shared class enumerating named topics the way `shortcutRegistry` holds two `Set`s — its two topics are both facts about the one subsystem `shared/keyboard/` already owns end to end, which isn't precedent for a central class that would need to know the string `tagDeleted`; that knowledge would put tags' domain vocabulary in the one layer this app has repeatedly kept blind to it (same reasoning as moving `Category` out of `api/` into `model/`, and as never letting the removed shared tag-by-id loader live in `shared/`). Deliberately payload-carrying, unlike `sessionStore`/`themeStore`'s zero-arg `Listener` + `getSnapshot()` — those work because session/theme have a real persisted current value; "a tag was deleted" is an occurrence, not state, and forcing it through a snapshot would mean inventing a fake current value (a last-event-plus-counter, which is `reloadToken` again, just generic). `useTopic` mirrors `useShortcut.ts`'s shape instead (a `useRef` kept current every render, a second effect keyed only on the topic subscribing once) because it's a registration/action hook, not a read hook — `useSyncExternalStore` never enters it, consistent with `useShortcut` needing none either. `tagDeleted` stays scoped to delete alone rather than a coarser `tagsChanged` anticipating create/edit's return — same bias against building for a consumer that doesn't exist yet as `entities/`, the tag-by-id loader, and `color-swatch-picker`. Net effect on the row above's design: `TagListOutletContext` narrows to `{ tags }` only, `TagDeletePage` no longer receives or needs a `reload` handle at all, and `useTagListPage` self-subscribes via `useTopic(tagDeleted, ...)` instead of receiving a callback — `useTagListPage`'s own `reloadToken` mechanism itself did not survive this either, see the row below. No ADR — same lighter §5-only treatment `shared/ui/confirm-dialog` got for the same kind of claim ("built to be reusable by the next flow, not feature-specific debris"). |
| `useTagListPage`'s `reloadToken` state removed; `tagsRepository.list()` is now called through a single `fetchTags(forQuery)` function (wrapped in `useCallback`), invoked directly from both the `[query]` effect and the `tagDeleted` subscription, instead of the query-change effect being the only fetch site and a reload being simulated by bumping a counter the effect also depended on | Explicit push-back, not a self-found improvement: `setReloadToken((token) => token + 1)` inside `reload()` read as an indirect trick — bump unrelated state purely so React notices the effect's deps changed and reruns it — when the actual desire was simpler, "call the fetch function directly." The one thing `reloadToken`-in-the-deps-array was quietly buying was race safety: bumping it forced React to tear down the previous effect instance (running its cleanup, setting that instance's own `ignore = true`) before the next fetch started, so a stale response could never land after a fresher one. Calling `fetchTags` directly from `useTopic`'s subscription bypasses the effect system entirely, so that cleanup-driven cancellation no longer fires for a reload — losing it silently would have reopened exactly the race `ignore` and, before it, [ADR 0033](../../../docs/adr/0033-admin-frontend-no-request-cancellation-layer.md) exist to close. Replaced with a `latestRequestRef` counter (a ref, not state) that every `fetchTags` call — from either trigger — increments and stamps its own response with; a response is applied only if its stamp still matches the ref when it resolves. This is strictly more robust than the row it replaces, not just simpler: the old per-effect `ignore` flag only ever guarded races _within_ the query-change effect's own reruns, because a reload never went through that effect at all in the naive reading — it only worked because `reloadToken` was deliberately smuggled into the effect's deps to force a rerun. The new guard protects both triggers uniformly, with the ordering rule stated once (`requestId !== latestRequestRef.current`) instead of emerging from effect-cleanup timing. Traded away: `status` no longer flips to `'loading'` while a reload is in flight — `isCurrent` now compares `query` alone, and `query` doesn't change on a reload, so the list keeps showing its (about-to-be-corrected) stale rows until the fresh response lands, rather than showing the skeleton. No test asserted that skeleton-during-reload behavior (only skeleton-during-search-refetch, a different path, left untouched), and it was this row's own side effect of reusing the search-loading derivation for reload, not something asked for — dropping it was judged worth the simplification. `fetchTags` needs `useCallback` (empty deps — it closes only over the stable `setResult` setter, the stable `latestRequestRef`, and its own `forQuery` parameter, never the component's `query` directly) so `react-hooks/exhaustive-deps` can accept `[query, fetchTags]` on the effect without `fetchTags`'s per-render identity forcing a rerun on every render. |
| `latestRequestRef` (the row above) replaced by a real `AbortController`, threaded through `tagsRepository.list(search, signal)` and `servicesFacade.ts`'s `CallOptions`/`LooseOptions`/`buildInit` — a scoped, explicit exception to [ADR 0033](../../../docs/adr/0033-admin-frontend-no-request-cancellation-layer.md), not a reversal of it | Requested directly, not motivated by a bug — `/tags` meets neither of ADR 0033's own stated bars for revisiting ("search-as-you-type, or a large export/report"): search here is Enter/button-submitted, and the catalog is small. The counter it replaces already fully closed the out-of-order-response race; nothing was broken. Shipped anyway, correctly, because ADR 0033's own two objections to the earlier attempt don't apply to this shape: `signal` is a single optional field threaded through three points in one shared file, not a parameter every repository method had to grow, and there is no `useApiResource`-style hook conflating fetching, cancellation and local state — `fetchTags` owns all three itself, in one place, same as before. `AbortController` **prevents** the race instead of **detecting** it after the fact: aborting the previous controller before a new one starts means a superseded request's success branch can never fire at all. Since `servicesApi` still never rejects (a core, load-bearing invariant — an aborted `fetch()` still resolves through `run()`'s `catch`, today as `fail(NETWORK_PROBLEM)`), the consumer checks `controller.signal.aborted` directly, before ever reading `apiResult`, rather than trusting the facade to classify its own cancellation correctly — §6 tracks that classification gap as a deliberate non-fix, narrow and harmless for this one caller. The `[query]` effect keeps its own cleanup (`abortControllerRef.current?.abort()`) specifically for the case `fetchTags`'s own "abort the previous controller" logic can't reach: unmounting, where there is no _next_ call to trigger it — this is the exact unmounted-component race ADR 0033 names as the one real bug worth guarding, now guarded by cancellation instead of by the `ignore`-flag pattern it credits. A short note was added to ADR 0033's own Consequences section pointing at this row, rather than a new ADR or a revert of 0033 — the policy it states ("no request-cancellation layer... revisit when a concrete need appears... scope the change to that endpoint") is still correct and still the default; this is the scoped exception it already anticipated, not a change of mind about the rule. `TagListPage.test.tsx`'s assertion on `tagsRepository.list`'s exact call args (`toHaveBeenCalledWith('vip')`) needed a second argument added (`expect.any(AbortSignal)`) — the one test in the suite that pinned the repository call's exact signature. A new test resolves two overlapping searches out of order (the newer one first) and asserts the stale one landing late never overwrites it — verified to actually fail without the `controller.signal.aborted` guard before being kept, not just asserted to pass. |
...
| `run()` classifying an aborted request's own `AbortError` as a distinct outcome, instead of folding it into `NETWORK_PROBLEM`        | `servicesFacade.ts`'s `CallOptions`/`buildInit` accept an optional `signal` (added for `useTagListPage`'s `fetchTags`, ADR 0033's scoped exception, §5), but the one consumer that passes one checks `controller.signal.aborted` itself, before ever looking at the `ApiResult` — it never needs `.error.code` to say "cancelled" correctly | A second `signal`-aware caller needs to tell "I cancelled this" apart from "the network actually failed" via the `ApiResult` itself, not by holding its own `AbortController` reference the way `fetchTags` does |
