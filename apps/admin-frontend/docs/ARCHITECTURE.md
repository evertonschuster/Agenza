...
the one compile-time checkpoint against a breaking wire change. `list` is the repository's only
function today; `get` / `create` / `update` / `remove` existed briefly for a create/edit/delete UI
that was later pulled back out (§5) — the backend still serves all five `/tags` endpoints, this
repository just doesn't call the other four.
...
against the last one a response resolved for (§5). Every failure maps to `'error'` the same way,
`Session.Missing` and `Authorization.Unauthorized` included — no redirect, no fourth outcome. A
listing hook's job is listing; deciding what an expired session means for navigation is not it (§5).
That is **not** the same as an expired session being handled somewhere else in the app — it currently
isn't. `ProtectedRoute` only reads the OIDC session snapshot in `shared/session`, which an API call
coming back 401 never touches, so nothing today reacts to those two codes by sending anyone to
`/login`. A page that needs that has to implement it itself, same as `TagListPage` used to before
this change — §5 has the finding, left as a known gap rather than papered over.
...
| No server-state library (TanStack Query, SWR, …)                                                                                                     | A repository call from a `loader`, an `action`, or a page's own hook all cover one screen equally well without one; when a query lib lands it **replaces** the repository — [ADR 0035](../../../docs/adr/0035-admin-frontend-no-server-state-library.md). `features/tags` cycled through three shapes for the exact same list screen (§5 has all three, most recently the `route.ts`/loader removal): a single-route `loader`/`action` (reverted, `5e48593`, no cross-route need yet); a list-only `loader` once `/tags` briefly split into four routes needing cross-route revalidation (removed with those routes); and, currently, a plain `useEffect` fetch in `useTagListPage.ts`, the same shape as the very first version. No query library needed at any point in that cycle — the churn was over which non-library shape fit the screen's routes that week, not whether one was missing. |
...
| Backend gained `GET /api/v{version}/tags/{id}`; `TagFormPage`(edit)/`TagRemovePage` stopped resolving their tag via `useRouteLoaderData('tags-list')` and now fetch it directly through a new shared `ui/pages/tagByIdLoader.ts` — the only thing these routes still share with the list is the `:id` in the URL | The row above shipped `useRouteLoaderData` because the backend had no single-tag `GET` at the time — reading from the list's already-loaded data was the only option, not the preferred one. That stops being merely impure and starts being wrong the moment the list's result set can genuinely diverge from "the tag this route needs" — a search-filtered `/tags?q=...` load has no obligation to contain every tag a stale `/tags/:id/edit` link points at. The backend addition mirrors `Categories`' `GetCategoryById` 1:1 (`ServicesService.Application/Tags/GetTagById/`, `TagsController.GetById` inserted List→Create→GetById→Update→Delete to match `CategoriesController`'s ordering, no query validator — matching the `GetCategoryById` precedent of relying on the `{id:guid}` route constraint plus a natural repository miss instead of a `NotEmpty()` rule). `tagByIdLoader` resolves the same three-outcome shape `tagListLoader` already established (`TagLoadResult = {status:'ready',tag} \| {status:'not-found'} \| {status:'error'}`, not thrown for an ordinary failure, for the same §2 reason list-loading avoids `unwrapOrThrow`) and lives at `ui/pages/tagByIdLoader.ts`, sibling to the page folders rather than inside `TagFormPage/` or `TagRemovePage/`, because it's wired onto both `:id/edit` and `:id/remove`'s route entries and belongs to neither page more than the other — duplicating it risks two copies drifting apart, and re-exporting from one page's `route.ts` into the other's would misname it after whichever page happened to define it first. `TagUnavailableDialog` (renamed from `TagNotFoundDialog`) now covers a `reason: 'not-found' \| 'error'`: the `error` case is genuinely new — reading from an already-successful parent load meant a broken fetch was never a state either page had to represent, but a real per-item network call means it now can be, covered with the established `shared/ui/error-state` retry pattern (`onRetry` → `revalidator.revalidate()`). |
| `/tags` reverted from the four-route CRUD split back to a single list-only route | Create, edit and delete were pulled from the app; `/tags` only lists. Everything the two rows above built on top of listing — `TagFormPage`/`TagRemovePage`, `tagByIdLoader`, `TagUnavailableDialog`, `TagFormFields`, all of `model/tagForm.ts`, and `shared/ui/color-swatch-picker` (no consumer left, and unlike `confirm-dialog` it had no test of its own to keep it honest in isolation — see that row above) — had no remaining caller once the decision landed, so it came out with the routes rather than staying as dead code the next reader has to independently confirm is unused. `tagsRepository`/`model/tag.ts` are trimmed to `list`/`Tag` only; `app/routes.tsx`'s `tags` route dropped its `new` / `:id/edit` / `:id/remove` children and its now-pointless `id: 'tags-list'` (nothing read it via `useRouteLoaderData` once the sibling routes were gone). `shared/ui/confirm-dialog` was left alone deliberately — it has its own generic, tag-free unit tests and was built explicitly to be reusable by the next delete-confirmation flow, not tag-specific debris. The backend keeps `GetById`/`Create`/`Update`/`Delete` on `TagsController` untouched — removing backend behavior that may have other consumers is a separate call from keeping this app's own UI honest about what it currently does. |
...
| Broad OpenAPI client generation                                                                                                      | `features/tags` calls one `/tags` endpoint; the rest of the generated surface (including the other four `/tags` operations the backend still exposes) is unused                                                                                                                                                                                                                                                                                                                  | The next business feature is wired                                                                                                       |
...
| Shared handler for a session that goes invalid mid-visit (`Session.Missing` / `Authorization.Unauthorized` from a live API response) | Confirmed by reading `apiClient`, `servicesFacade`, `ProtectedRoute` and `shared/session` end to end: none of them react to this today — `ProtectedRoute` only redirects off the OIDC snapshot, which a 401 from `services-service` never touches. `useTagListPage` used to redirect on these two codes itself; that was removed on the explicit call that a listing hook's job is listing, not session lifecycle (§5) — so right now nothing sends this person back to `/login` | A second page starts fetching past a session that can go stale mid-visit — build it once, in `shared/api`/`shared/session`, not per page |
